...
public class SharedClientDataSource {
	...
	public SharedClientView getById (long id) {
		...
		Cursor cursor = context.getContentResolver().query(SHARED_CLIENT_BY_ID, null, null, new String[]{ String.valueOf(id) }, null);
		if (null != cursor) {
			cursor.moveToFirst();
	...
	public long getLastId () {
		long result = 0;
		Cursor cursor = context.getContentResolver().query(SHARED_CLIENT_LAST_ID, null, null, null, null);
		if (null != cursor) {
			cursor.moveToFirst();
		    if(!cursor.isAfterLast()){
