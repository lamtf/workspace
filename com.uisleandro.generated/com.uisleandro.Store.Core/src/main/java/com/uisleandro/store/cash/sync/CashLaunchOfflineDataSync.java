...
import com.uisleandro.store.cash.model.CashLaunchOfflineHelper;
...
public class CashLaunchOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	CashLaunchOfflineHelper cashLaunchOfflineHelper;

	public CashLaunchSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		cashLaunchOfflineHelper = new CashLaunchOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = cashLaunchOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/cash_launch/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								CashLaunch cash_launch = CashLaunch.FromJsonObj(array.getJSONObject(i));
								cashLaunchOfflineHelper.insert(cash_launch); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			cashLaunchOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = cashLaunchOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/cash_launch/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										CashLaunch obj = CashLaunch.FromJsonObj(
												array.getJSONObject(i)
										);
										cashLaunchOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				cashLaunchOfflineHelper.after_client_updating();
			}
	
	}

	public void fix_foreign_keys_on_client(){
	cashLaunchOfflineHelper.fixClientForeignKeys();
	}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<CashLaunch> cash_launch_for_insertion_on_server =
					cashLaunchOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<CashLaunch> it = cash_launch_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/cash_launch/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							cashLaunchOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<CashLaunch> update_remote = cashLaunchOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<CashLaunch> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/cash_launch/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
