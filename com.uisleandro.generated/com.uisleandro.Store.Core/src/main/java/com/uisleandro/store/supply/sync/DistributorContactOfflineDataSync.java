...
import com.uisleandro.store.supply.model.DistributorContactOfflineHelper;
...
public class DistributorContactOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	DistributorContactOfflineHelper distributorContactOfflineHelper;

	public DistributorContactSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		distributorContactOfflineHelper = new DistributorContactOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = distributorContactOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/distributor_contact/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								DistributorContact distributor_contact = DistributorContact.FromJsonObj(array.getJSONObject(i));
								distributorContactOfflineHelper.insert(distributor_contact); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			distributorContactOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = distributorContactOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/distributor_contact/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										DistributorContact obj = DistributorContact.FromJsonObj(
												array.getJSONObject(i)
										);
										distributorContactOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				distributorContactOfflineHelper.after_client_updating();
			}
	
	}

	public void fix_foreign_keys_on_client(){
	distributorContactOfflineHelper.fixClientForeignKeys();
	}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<DistributorContact> distributor_contact_for_insertion_on_server =
					distributorContactOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<DistributorContact> it = distributor_contact_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/distributor_contact/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							distributorContactOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<DistributorContact> update_remote = distributorContactOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<DistributorContact> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/distributor_contact/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
