...
import com.uisleandro.store.client.model.BasicClientOfflineHelper;
...
public class BasicClientOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	BasicClientOfflineHelper basicClientOfflineHelper;

	public BasicClientSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		basicClientOfflineHelper = new BasicClientOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = basicClientOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/basic_client/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								BasicClient basic_client = BasicClient.FromJsonObj(array.getJSONObject(i));
								basicClientOfflineHelper.insert(basic_client); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			basicClientOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = basicClientOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/basic_client/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										BasicClient obj = BasicClient.FromJsonObj(
												array.getJSONObject(i)
										);
										basicClientOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				basicClientOfflineHelper.after_client_updating();
			}
	
	}

	public void fix_foreign_keys_on_client(){
	basicClientOfflineHelper.fixClientForeignKeys();
	}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<BasicClient> basic_client_for_insertion_on_server =
					basicClientOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<BasicClient> it = basic_client_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/basic_client/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							basicClientOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<BasicClient> update_remote = basicClientOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<BasicClient> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/basic_client/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
