...
import com.uisleandro.store.core.model.RoleOfflineHelper;
...
public class RoleOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	RoleOfflineHelper roleOfflineHelper;

	public RoleSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		roleOfflineHelper = new RoleOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = roleOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/role/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								Role role = Role.FromJsonObj(array.getJSONObject(i));
								roleOfflineHelper.insert(role); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			roleOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = roleOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/role/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										Role obj = Role.FromJsonObj(
												array.getJSONObject(i)
										);
										roleOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				roleOfflineHelper.after_client_updating();
			}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<Role> role_for_insertion_on_server =
					roleOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<Role> it = role_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/role/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							roleOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<Role> update_remote = roleOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<Role> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/role/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
