...
import com.uisleandro.store.receivement.model.InstallmentTypeOfflineHelper;
...
public class InstallmentTypeOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	InstallmentTypeOfflineHelper installmentTypeOfflineHelper;

	public InstallmentTypeSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		installmentTypeOfflineHelper = new InstallmentTypeOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = installmentTypeOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/installment_type/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								InstallmentType installment_type = InstallmentType.FromJsonObj(array.getJSONObject(i));
								installmentTypeOfflineHelper.insert(installment_type); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			installmentTypeOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = installmentTypeOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/installment_type/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										InstallmentType obj = InstallmentType.FromJsonObj(
												array.getJSONObject(i)
										);
										installmentTypeOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				installmentTypeOfflineHelper.after_client_updating();
			}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<InstallmentType> installment_type_for_insertion_on_server =
					installmentTypeOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<InstallmentType> it = installment_type_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/installment_type/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							installmentTypeOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<InstallmentType> update_remote = installmentTypeOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<InstallmentType> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/installment_type/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
