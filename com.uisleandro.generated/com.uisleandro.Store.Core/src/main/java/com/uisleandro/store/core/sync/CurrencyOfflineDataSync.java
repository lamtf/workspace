...
import com.uisleandro.store.core.model.CurrencyOfflineHelper;
...
public class CurrencyOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	CurrencyOfflineHelper currencyOfflineHelper;

	public CurrencySync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		currencyOfflineHelper = new CurrencyOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = currencyOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/currency/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								Currency currency = Currency.FromJsonObj(array.getJSONObject(i));
								currencyOfflineHelper.insert(currency); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			currencyOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = currencyOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/currency/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										Currency obj = Currency.FromJsonObj(
												array.getJSONObject(i)
										);
										currencyOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				currencyOfflineHelper.after_client_updating();
			}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<Currency> currency_for_insertion_on_server =
					currencyOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<Currency> it = currency_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/currency/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							currencyOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<Currency> update_remote = currencyOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<Currency> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/currency/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
