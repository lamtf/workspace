...
import com.uisleandro.store.receivement.model.InvoiceOfflineHelper;
...
public class InvoiceOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	InvoiceOfflineHelper invoiceOfflineHelper;

	public InvoiceSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		invoiceOfflineHelper = new InvoiceOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = invoiceOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/invoice/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								Invoice invoice = Invoice.FromJsonObj(array.getJSONObject(i));
								invoiceOfflineHelper.insert(invoice); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			invoiceOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = invoiceOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/invoice/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										Invoice obj = Invoice.FromJsonObj(
												array.getJSONObject(i)
										);
										invoiceOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				invoiceOfflineHelper.after_client_updating();
			}
	
	}

	public void fix_foreign_keys_on_client(){
	invoiceOfflineHelper.fixClientForeignKeys();
	}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<Invoice> invoice_for_insertion_on_server =
					invoiceOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<Invoice> it = invoice_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/invoice/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							invoiceOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<Invoice> update_remote = invoiceOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<Invoice> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/invoice/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
