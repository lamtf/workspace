package com.uisleandro.store.receivement.sync;

import android.content.Context;
import android.util.Log;

import com.uisleandro.util.DBPaginator;
import com.uisleandro.store.receivement.model.InterestRateTypeOfflineHelper;
...
public class InterestRateTypeOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	InterestRateTypeOfflineHelper interestRateTypeOfflineHelper;

	public InterestRateTypeSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		interestRateTypeOfflineHelper = new InterestRateTypeOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = interestRateTypeOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/interest_rate_type/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								InterestRateType interest_rate_type = InterestRateType.FromJsonObj(array.getJSONObject(i));
								interestRateTypeOfflineHelper.insert(interest_rate_type); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			interestRateTypeOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = interestRateTypeOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/interest_rate_type/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										InterestRateType obj = InterestRateType.FromJsonObj(
												array.getJSONObject(i)
										);
										interestRateTypeOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				interestRateTypeOfflineHelper.after_client_updating();
			}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<InterestRateType> interest_rate_type_for_insertion_on_server =
					interestRateTypeOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<InterestRateType> it = interest_rate_type_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/interest_rate_type/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							interestRateTypeOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<InterestRateType> update_remote = interestRateTypeOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<InterestRateType> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/interest_rate_type/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
