package com.uisleandro.store.sales.sync;

import android.content.Context;
import android.util.Log;

import com.uisleandro.util.DBPaginator;
import com.uisleandro.store.sales.model.ProductOnSaleOfflineHelper;
...
public class ProductOnSaleOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	ProductOnSaleOfflineHelper productOnSaleOfflineHelper;

	public ProductOnSaleSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		productOnSaleOfflineHelper = new ProductOnSaleOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = productOnSaleOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/product_on_sale/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								ProductOnSale product_on_sale = ProductOnSale.FromJsonObj(array.getJSONObject(i));
								productOnSaleOfflineHelper.insert(product_on_sale); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			productOnSaleOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = productOnSaleOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/product_on_sale/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										ProductOnSale obj = ProductOnSale.FromJsonObj(
												array.getJSONObject(i)
										);
										productOnSaleOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				productOnSaleOfflineHelper.after_client_updating();
			}
	
	}

	public void fix_foreign_keys_on_client(){
	productOnSaleOfflineHelper.fixClientForeignKeys();
	}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<ProductOnSale> product_on_sale_for_insertion_on_server =
					productOnSaleOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<ProductOnSale> it = product_on_sale_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/product_on_sale/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							productOnSaleOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<ProductOnSale> update_remote = productOnSaleOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<ProductOnSale> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/product_on_sale/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
