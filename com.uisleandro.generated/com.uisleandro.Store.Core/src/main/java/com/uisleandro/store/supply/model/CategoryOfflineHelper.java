...
public class CategoryOfflineHelper {

	private Context context;
	private SQLiteDatabase database;
	private DbHelper db_helper;

	public CategoryOfflineHelper (Context context) {
		this.context = context;
		db_helper = DbHelper.getInstance(context);
		try{
			database = db_helper.getWritableDatabase();
		}catch(SQLException e){
			Log.wtf("CategoryOfflineHelper", "Exception: "+Log.getStackTraceString(e));
		}
	}
	...
	public long insert(CategoryDataView that){
	...
	public int update(CategoryDataView that){
		ContentValues values = new ContentValues();
		if(that.getServerId() > 0){
			values.put(DbHelper.CATEGORY_SERVER_ID, that.getServerId());
		}
	...
	public List<CategoryDataView> listForInsertOnServer(long page_count, long page_size){

		String query = "SELECT t0.id, t0.server_id, t0.dirty, "+
		"t0.last_update, " +
		"t2.server_id as fk_category, " +
		"t0.name" +
		" FROM "+DbHelper.TABLE_CATEGORY+" t0" +
		" INNER JOIN "+DbHelper.TABLE_CATEGORY+" t1 ON t0.fk_category = t1.id";		query += " WHERE t0.server_id IS NULL";
		if(page_size > 0){
			query += " LIMIT " + String.valueOf(page_size) + " OFFSET " + String.valueOf(page_size * page_count);
		}
		query += ";";
		Log.wtf("rest-api", query);
		List<CategoryDataView> those = new ArrayList<>();
		Cursor cursor = database.rawQuery(query, null);
		cursor.moveToFirst();
	    while(!cursor.isAfterLast()){
	      those.add(CategoryDataView.FromCursor(cursor));
	      cursor.moveToNext();
	    }
	    cursor.close();
		return those;
	...
	public List<CategoryDataView> listForUpdateOnServer(long page_count, long page_size){
		...
		"t0.name" +
		" FROM "+DbHelper.TABLE_CATEGORY+" t0" +
		" INNER JOIN "+DbHelper.TABLE_CATEGORY+" t1 ON t0.fk_category = t1.id";		query += " WHERE t0." + DbHelper.CATEGORY_DIRTY + " = 1";
		if(page_size > 0) {
			query += " LIMIT " + String.valueOf(page_size) + " OFFSET " + String.valueOf(page_size * page_count);
		}
		query += ";";
	...
	public int fixAfterServerInsertAndUpdate(long local_id, long remote_id, long last_update_time){
		...
		int rows_affected = database.update(
			...
			values,
			DbHelper.CATEGORY_ID + " = " + String.valueOf(local_id),
		null);
		return rows_affected;
	}
	...
	public long getLastServerId(){
		long result = 0;
		String query = "SELECT MAX(server_id) FROM " + DbHelper.TABLE_CATEGORY +";";
		Cursor cursor = database.rawQuery(query, null);
	...
	public long getLastUpdateTime(){
		long result = 0;
		String query = "SELECT last_update_time FROM " + DbHelper.TABLE_UPDATE_HISTORY +" WHERE table_name = '"+DbHelper.TABLE_CATEGORY+"';";
		Cursor cursor = database.rawQuery(query, null);
		long res = cursor.getLong(0);
		cursor.close();
		return res;
	...
	public void before_client_updating(){
		String query = "UPDATE " + DbHelper.TABLE_UPDATE_HISTORY + " SET last_update_time = ( SELECT MAX(last_update_time) FROM " +
			DbHelper.TABLE_CATEGORY + " ) WHERE table_name = '" + DbHelper.TABLE_CATEGORY + "' AND last_update_time IS NULL;";
		database.rawQuery(query, null);
	}

	//set the last_update_time, from this table, to null
	public void after_client_updating(){
		String query = "UPDATE " + DbHelper.TABLE_UPDATE_HISTORY + " SET last_update_time = NULL WHERE table_name = '" + DbHelper.TABLE_CATEGORY + "';";
		database.rawQuery(query, null);
	}
	...
	public int fixClientForeignKeys(){
		String query = "UPDATE " + DbHelper.TABLE_CATEGORY + " SET "+
		"fk_category = ( SELECT id FROM " + DbHelper.TABLE_CATEGORY + " WHERE " + DbHelper.TABLE_CATEGORY + ".server_id = " + DbHelper.TABLE_CATEGORY + ".fk_category );";		int result = 0;
		Cursor cursor = database.rawQuery(query, null);
