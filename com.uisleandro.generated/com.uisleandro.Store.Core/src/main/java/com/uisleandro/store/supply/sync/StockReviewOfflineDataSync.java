...
import com.uisleandro.store.supply.model.StockReviewOfflineHelper;
...
public class StockReviewOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	StockReviewOfflineHelper stockReviewOfflineHelper;

	public StockReviewSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		stockReviewOfflineHelper = new StockReviewOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = stockReviewOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/stock_review/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								StockReview stock_review = StockReview.FromJsonObj(array.getJSONObject(i));
								stockReviewOfflineHelper.insert(stock_review); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			stockReviewOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = stockReviewOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/stock_review/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										StockReview obj = StockReview.FromJsonObj(
												array.getJSONObject(i)
										);
										stockReviewOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				stockReviewOfflineHelper.after_client_updating();
			}
	
	}

	public void fix_foreign_keys_on_client(){
	stockReviewOfflineHelper.fixClientForeignKeys();
	}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<StockReview> stock_review_for_insertion_on_server =
					stockReviewOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<StockReview> it = stock_review_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/stock_review/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							stockReviewOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<StockReview> update_remote = stockReviewOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<StockReview> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/stock_review/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
