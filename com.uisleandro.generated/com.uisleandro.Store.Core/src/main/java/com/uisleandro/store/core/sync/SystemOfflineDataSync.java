...
import com.uisleandro.store.core.model.SystemOfflineHelper;
...
public class SystemOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	SystemOfflineHelper systemOfflineHelper;

	public SystemSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		systemOfflineHelper = new SystemOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = systemOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/system/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								System system = System.FromJsonObj(array.getJSONObject(i));
								systemOfflineHelper.insert(system); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			systemOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = systemOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/system/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										System obj = System.FromJsonObj(
												array.getJSONObject(i)
										);
										systemOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				systemOfflineHelper.after_client_updating();
			}
	
	}

	public void fix_foreign_keys_on_client(){
	systemOfflineHelper.fixClientForeignKeys();
	}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<System> system_for_insertion_on_server =
					systemOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<System> it = system_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/system/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							systemOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<System> update_remote = systemOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<System> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/system/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
