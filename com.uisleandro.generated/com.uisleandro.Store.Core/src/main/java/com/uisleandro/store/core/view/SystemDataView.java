...
public class SystemDataView {
	...
	public long getFkReseller () {
		return fk_reseller;
	}

	public void setFkReseller (long fk_reseller) {
		this.fk_reseller = fk_reseller;
	}
	...
	public static SystemDataView FromJsonObj(JSONObject obj) {
		if(null != obj) {
			try {
				...
				if(obj.has("fk_reseller") && !obj.isNull("fk_reseller")){
					result.setFkReseller(obj.getLong("fk_reseller"));
				}
				return result;
			} catch (JSONException e) {
				e.printStackTrace();
	...
	public static SystemDataView FromCursor (Cursor cursor) {
		if(null != cursor){
			...
			result.setFkReseller(cursor.getLong(16));
			return result;		
		}
		return null;
	}
	...
	public String[] toInsertArray () {
		return new String[]{String.valueOf(last_update), name, String.valueOf(enabled), String.valueOf(fk_currency), fantasy_name, stores_address, srores_address_number, stores_city, stores_neighborhood, stores_zip_code, stores_state, stores_email, stores_phonenumber, String.valueOf(fk_reseller)};
	}

	public String[] toUpdateArray () {
		return new String[]{String.valueOf(id), String.valueOf(server_id), String.valueOf(dirty), String.valueOf(last_update), name, String.valueOf(enabled), String.valueOf(fk_currency), fantasy_name, stores_address, srores_address_number, stores_city, stores_neighborhood, stores_zip_code, stores_state, stores_email, stores_phonenumber, String.valueOf(fk_reseller)};
	}
