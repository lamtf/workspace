...
import com.uisleandro.store.client.model.BrazilianOfflineHelper;
...
public class BrazilianOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	BrazilianOfflineHelper brazilianOfflineHelper;

	public BrazilianSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		brazilianOfflineHelper = new BrazilianOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = brazilianOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/brazilian/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								Brazilian brazilian = Brazilian.FromJsonObj(array.getJSONObject(i));
								brazilianOfflineHelper.insert(brazilian); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			brazilianOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = brazilianOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/brazilian/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										Brazilian obj = Brazilian.FromJsonObj(
												array.getJSONObject(i)
										);
										brazilianOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				brazilianOfflineHelper.after_client_updating();
			}
	
	}

	public void fix_foreign_keys_on_client(){
	brazilianOfflineHelper.fixClientForeignKeys();
	}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<Brazilian> brazilian_for_insertion_on_server =
					brazilianOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<Brazilian> it = brazilian_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/brazilian/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							brazilianOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<Brazilian> update_remote = brazilianOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<Brazilian> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/brazilian/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
