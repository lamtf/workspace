...
public class CashRegisterOfflineHelper {
	...
	public List<CashRegisterDataView> listForUpdateOnServer(long page_count, long page_size){
		...
		" FROM "+DbHelper.TABLE_CASH_REGISTER+" t0" +
		" INNER JOIN "+DbHelper.TABLE_USER+" t1 ON t0.fk_user = t1.id";
		query += " WHERE t0." + DbHelper.CASH_REGISTER_DIRTY + " = 1";
	...
	public int fixClientForeignKeys(){
		...
		"fk_user = ( SELECT id FROM " + DbHelper.TABLE_USER + " WHERE " + DbHelper.TABLE_USER + ".server_id = " + DbHelper.TABLE_CASH_REGISTER + ".fk_user );";
