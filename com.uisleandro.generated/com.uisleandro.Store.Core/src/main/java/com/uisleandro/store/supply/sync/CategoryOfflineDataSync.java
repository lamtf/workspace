...
import com.uisleandro.store.supply.model.CategoryOfflineHelper;
...
public class CategoryOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	CategoryOfflineHelper categoryOfflineHelper;

	public CategorySync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		categoryOfflineHelper = new CategoryOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = categoryOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/category/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								Category category = Category.FromJsonObj(array.getJSONObject(i));
								categoryOfflineHelper.insert(category); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			categoryOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = categoryOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/category/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										Category obj = Category.FromJsonObj(
												array.getJSONObject(i)
										);
										categoryOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				categoryOfflineHelper.after_client_updating();
			}
	
	}

	public void fix_foreign_keys_on_client(){
	categoryOfflineHelper.fixClientForeignKeys();
	}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<Category> category_for_insertion_on_server =
					categoryOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<Category> it = category_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/category/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							categoryOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<Category> update_remote = categoryOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<Category> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/category/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
