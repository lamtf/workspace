...
import com.uisleandro.store.core.model.TokenOfflineHelper;
...
public class TokenOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	TokenOfflineHelper tokenOfflineHelper;

	public TokenSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		tokenOfflineHelper = new TokenOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = tokenOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/token/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								Token token = Token.FromJsonObj(array.getJSONObject(i));
								tokenOfflineHelper.insert(token); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			tokenOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = tokenOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/token/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										Token obj = Token.FromJsonObj(
												array.getJSONObject(i)
										);
										tokenOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				tokenOfflineHelper.after_client_updating();
			}
	
	}

	public void fix_foreign_keys_on_client(){
	tokenOfflineHelper.fixClientForeignKeys();
	}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<Token> token_for_insertion_on_server =
					tokenOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<Token> it = token_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/token/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							tokenOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<Token> update_remote = tokenOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<Token> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/token/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
