...
public class SharedClientOfflineHelper {
	...
	public List<SharedClientDataView> listForUpdateOnServer(long page_count, long page_size){
		...
		" FROM "+DbHelper.TABLE_SHARED_CLIENT+" t0" +
		" INNER JOIN "+DbHelper.TABLE_COUNTRY+" t1 ON t0.fk_country = t1.id";
		query += " WHERE t0." + DbHelper.SHARED_CLIENT_DIRTY + " = 1";
	...
	public int fixClientForeignKeys(){
		...
		"fk_country = ( SELECT id FROM " + DbHelper.TABLE_COUNTRY + " WHERE " + DbHelper.TABLE_COUNTRY + ".server_id = " + DbHelper.TABLE_SHARED_CLIENT + ".fk_country );";
