...
public class SaleTypeOfflineHelper {

	private Context context;
	private SQLiteDatabase database;
	private DbHelper db_helper;

	public SaleTypeOfflineHelper (Context context) {
		this.context = context;
		db_helper = DbHelper.getInstance(context);
		try{
			database = db_helper.getWritableDatabase();
		}catch(SQLException e){
			Log.wtf("SaleTypeOfflineHelper", "Exception: "+Log.getStackTraceString(e));
		}
	}
	...
	public long insert(SaleTypeDataView that){
	...
	public List<SaleTypeDataView> listForInsertOnServer(long page_count, long page_size){

		String query = "SELECT t0.id, t0.server_id, t0.dirty, "+
		"t0.last_update, " +
		"t0.name" +
		" FROM "+DbHelper.TABLE_SALE_TYPE+" t0";
	...
	public List<SaleTypeDataView> listForUpdateOnServer(long page_count, long page_size){
		...
		"t0.name" +
		" FROM "+DbHelper.TABLE_SALE_TYPE+" t0";
		query += " WHERE t0." + DbHelper.SALE_TYPE_DIRTY + " = 1";
		if(page_size > 0) {
			query += " LIMIT " + String.valueOf(page_size) + " OFFSET " + String.valueOf(page_size * page_count);
		}
		query += ";";
	...
	public int fixAfterServerInsertAndUpdate(long local_id, long remote_id, long last_update_time){
		ContentValues values = new ContentValues();
		values.put(DbHelper.SALE_TYPE_SERVER_ID, remote_id);
		values.put(DbHelper.SALE_TYPE_LAST_UPDATE_TIME, last_update_time);
		values.put(DbHelper.SALE_TYPE_DIRTY, 0);
		int rows_affected = database.update(
			DbHelper.TABLE_SALE_TYPE,
			values,
			DbHelper.SALE_TYPE_ID + " = " + String.valueOf(local_id),
		null);
		return rows_affected;
	}
	...
	public long getLastServerId(){
		long result = 0;
		String query = "SELECT MAX(server_id) FROM " + DbHelper.TABLE_SALE_TYPE +";";
		Cursor cursor = database.rawQuery(query, null);
	...
	public long getLastUpdateTime(){
		long result = 0;
		String query = "SELECT last_update_time FROM " + DbHelper.TABLE_UPDATE_HISTORY +" WHERE table_name = '"+DbHelper.TABLE_SALE_TYPE+"';";
		Cursor cursor = database.rawQuery(query, null);
		long res = cursor.getLong(0);
		cursor.close();
		return res;
	...
	public void before_client_updating(){
		String query = "UPDATE " + DbHelper.TABLE_UPDATE_HISTORY + " SET last_update_time = ( SELECT MAX(last_update_time) FROM " +
			DbHelper.TABLE_SALE_TYPE + " ) WHERE table_name = '" + DbHelper.TABLE_SALE_TYPE + "' AND last_update_time IS NULL;";
		database.rawQuery(query, null);
	}

	//set the last_update_time, from this table, to null
	public void after_client_updating(){
		String query = "UPDATE " + DbHelper.TABLE_UPDATE_HISTORY + " SET last_update_time = NULL WHERE table_name = '" + DbHelper.TABLE_SALE_TYPE + "';";
		database.rawQuery(query, null);
	}
