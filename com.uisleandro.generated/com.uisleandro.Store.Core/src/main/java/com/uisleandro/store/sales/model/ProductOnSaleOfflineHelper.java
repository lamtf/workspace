...
public class ProductOnSaleOfflineHelper {
	...
	public ProductOnSaleOfflineHelper (Context context) {
		...
		try{
			...
			Log.wtf("ProductOnSaleOfflineHelper", "Exception: "+Log.getStackTraceString(e));
		}
	}
	...
	public long insert(ProductOnSaleView that){
		...
		if(that.getFkFkSale() > 0){
			values.put(DbHelper.PRODUCT_ON_SALE_FK_SALE, that.getFkFkSale());
		}
		if(that.getFkFkProduct() > 0){
			values.put(DbHelper.PRODUCT_ON_SALE_FK_PRODUCT, that.getFkFkProduct());
		}
	...
	public List<ProductOnSaleDataView> listForInsertOnServer(long page_count, long page_size){
		...
		" INNER JOIN "+DbHelper.TABLE_PRODUCT+" t2 ON t0.fk_product = t2.id";		query += " WHERE t0.server_id IS NULL";
		if(page_size > 0){
	...
	public List<ProductOnSaleDataView> listForUpdateOnServer(long page_count, long page_size){
		...
		" FROM "+DbHelper.TABLE_PRODUCT_ON_SALE+" t0" +
		" INNER JOIN "+DbHelper.TABLE_SALE+" t1 ON t0.fk_sale = t1.id" +
		" INNER JOIN "+DbHelper.TABLE_PRODUCT+" t2 ON t0.fk_product = t2.id";		query += " WHERE t0." + DbHelper.PRODUCT_ON_SALE_DIRTY + " = 1";
		if(page_size > 0) {
			query += " LIMIT " + String.valueOf(page_size) + " OFFSET " + String.valueOf(page_size * page_count);
		}
		query += ";";
