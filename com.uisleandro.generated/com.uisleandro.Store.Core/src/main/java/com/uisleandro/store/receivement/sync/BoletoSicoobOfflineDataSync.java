...
import com.uisleandro.store.receivement.model.BoletoSicoobOfflineHelper;
...
public class BoletoSicoobOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	BoletoSicoobOfflineHelper boletoSicoobOfflineHelper;

	public BoletoSicoobSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		boletoSicoobOfflineHelper = new BoletoSicoobOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = boletoSicoobOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/boleto_sicoob/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								BoletoSicoob boleto_sicoob = BoletoSicoob.FromJsonObj(array.getJSONObject(i));
								boletoSicoobOfflineHelper.insert(boleto_sicoob); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			boletoSicoobOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = boletoSicoobOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/boleto_sicoob/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										BoletoSicoob obj = BoletoSicoob.FromJsonObj(
												array.getJSONObject(i)
										);
										boletoSicoobOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				boletoSicoobOfflineHelper.after_client_updating();
			}
	
	}

	public void fix_foreign_keys_on_client(){
	boletoSicoobOfflineHelper.fixClientForeignKeys();
	}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<BoletoSicoob> boleto_sicoob_for_insertion_on_server =
					boletoSicoobOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<BoletoSicoob> it = boleto_sicoob_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/boleto_sicoob/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							boletoSicoobOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<BoletoSicoob> update_remote = boletoSicoobOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<BoletoSicoob> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/boleto_sicoob/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
