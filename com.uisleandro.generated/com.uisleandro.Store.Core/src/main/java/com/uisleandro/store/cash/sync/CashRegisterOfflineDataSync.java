package com.uisleandro.store.cash.sync;

import android.content.Context;
import android.util.Log;

import com.uisleandro.util.DBPaginator;
import com.uisleandro.store.cash.model.CashRegisterOfflineHelper;
...
public class CashRegisterOfflineDataSync implements SyncUpdater {

	final DBPaginator paginator;
	TLSWebClient2 client;
	CashRegisterOfflineHelper cashRegisterOfflineHelper;

	public CashRegisterSync(TLSWebClient2 client, Context context){
		this.client = client;
		paginator = new DBPaginator();
		cashRegisterOfflineHelper = new CashRegisterOfflineHelper(context);
	}

	public void insert_on_client(){
		long the_last_server_id_on_client = cashRegisterOfflineHelper.getLastServerId(); //step 1,1
		...
		while (false == paginator.isFinished()) {
			...
			//step 1,2
			client.run(
				"https://192.168.0.250/naiara/sync/cash_register/get_for_client_insert", json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
						...
						try {
							...
							for (int i = 0; i < array.length(); i++) {
								...
								CashRegister cash_register = CashRegister.FromJsonObj(array.getJSONObject(i));
								cashRegisterOfflineHelper.insert(cash_register); //step 1,3
							}
	...
	public void update_on_client(){
		...
		paginator.setError(false);
			...
			cashRegisterOfflineHelper.before_client_updating();
			while (false == paginator.isFinished()) {
				long lastUpdateTime = cashRegisterOfflineHelper.getLastUpdateTime();
				String json = "{ " +
				...
				client.run("https://192.168.0.250/naiara/sync/cash_register/get_for_client_update", json, new TlsCallback() {
						@Override
						public void CallBack(String result) {
						Log.v("rest-api", "get_for_client_update :\n" + result);
							try {
								...
								if(0 < array.length()){
									for (int i = 0; i < array.length(); i++) {
										CashRegister obj = CashRegister.FromJsonObj(
												array.getJSONObject(i)
										);
										cashRegisterOfflineHelper.update(obj);
									}
								} else {
									paginator.setFinished(true);
								}
			...
			if(!paginator.isError()) {
				cashRegisterOfflineHelper.after_client_updating();
			}
	
	}

	public void fix_foreign_keys_on_client(){
	cashRegisterOfflineHelper.fixClientForeignKeys();
	}
	...
	public void insert_on_server(){
		...
		while (false == paginator.isFinished()) {
			List<CashRegister> cash_register_for_insertion_on_server =
					cashRegisterOfflineHelper.listForInsertOnServer(
							paginator.getPageNumber(),
							paginator.getPageSize()); //step 2,1
			paginator.nextPage();
			Iterator<CashRegister> it = cash_register_for_insertion_on_server.iterator();
			String json = "[";
			if(it.hasNext()){
			...
			//step 2,2
			client.run(
				"https://192.168.0.250/naiara/sync/cash_register/bash_insert",
				json,
				new TlsCallback() {
					@Override
					public void CallBack(String result) {
					...
					try {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							//step 2,3
							cashRegisterOfflineHelper.fixAfterServerInsertAndUpdate(
	...
	public void update_on_server(){
		...
		while (false == paginator.isFinished()) {
			//4,2
			List<CashRegister> update_remote = cashRegisterOfflineHelper.listForUpdateOnServer(paginator.getPageNumber(), paginator.getPageSize());
			paginator.nextPage();
			Iterator<CashRegister> it = update_remote.iterator();
			String json = "[";
			if (it.hasNext()) {
			...
			//4,3
			client.run(
				"https://192.168.0.250/naiara/sync/cash_register/bash_update", json, new TlsCallback() {
				@Override
				public void CallBack(String result) {
					Log.v("rest-api", "bash_update :\n" + result);
					try {
