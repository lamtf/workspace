...
class Role {
	...
	public function get_for_client_insert($fk_role, $page_number, $page_size){
		$query = "SELECT \"id\" as \"server_id\", \"last_update\"".
		",\"name\"".
		" FROM \"simpledb\".\"role\"".
		" WHERE \"id\" > ". \db::number($fk_role).
	...
	public function insert($last_update,$name){
		...
		$query = "INSERT INTO \"simpledb\".\"role\"(".
		"\"last_update\"".
		",\"name\"".") VALUES (".
		...
		$this->db->query($query);
		$query = "SELECT currval(pg_get_serial_sequence('simpledb.role','id'));";
		$stmt = $this->db->query($query);
	...
	public function update($id, $last_update,$name){
		...
		$query = "UPDATE \"simpledb\".\"role\" SET ".
	...
	function get_for_client_update($last_update_time, $page_number, $page_size){
		...
		$query = "SELECT \"id\" as \"server_id\", \"last_update\", \"name\" FROM \"simpledb\".\"role\" WHERE last_update_time > " . $last_update_time;
