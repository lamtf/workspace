...
class Brand {
	...
	public function get_for_client_insert($fk_brand, $page_number, $page_size){
		$query = "SELECT \"id\" as \"server_id\", \"last_update\"".
		",\"company_name\"".
		",\"fantasy_name\"".
		" FROM \"simpledb\".\"brand\"".
		" WHERE \"id\" > ". \db::number($fk_brand).
	...
	public function insert($last_update,$company_name,$fantasy_name){
		$last_update_time = \db::timestamp();

		$query = "INSERT INTO \"simpledb\".\"brand\"(".
		"\"last_update\"".
		",\"company_name\"".
		",\"fantasy_name\"".") VALUES (".
		\db::string($last_update) .
		"," .\db::string($company_name) .
		"," .\db::string($fantasy_name) .");";

		$this->db->query($query);
		$query = "SELECT currval(pg_get_serial_sequence('simpledb.brand','id'));";
		$stmt = $this->db->query($query);
	...
	public function update($id, $last_update,$company_name,$fantasy_name){

		$last_update_time = \db::timestamp();

		$query = "UPDATE \"simpledb\".\"brand\" SET ".
		"\"last_update\" = ".\db::string($last_update) .
		",\"company_name\" = ".\db::string($company_name) .
	...
	function get_for_client_update($last_update_time, $page_number, $page_size){
		...
		$query = "SELECT \"id\" as \"server_id\", \"last_update\", \"company_name\", \"fantasy_name\" FROM \"simpledb\".\"brand\" WHERE last_update_time > " . $last_update_time;
