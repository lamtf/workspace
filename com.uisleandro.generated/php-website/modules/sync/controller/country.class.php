...
use Sync\Model\Country as CountryModel;
...
class Country {
	...
	public function get_for_client_insert(){
		...
		if(isset($_REQUEST["json"])){
			...
			$result = $country->get_for_client_insert($obj->server_id, $obj->page_number, $obj->page_size);
	...
	function bash_insert(){
		...
		if(isset($_REQUEST["json"])){
			...
			for($i = 0; $i < sizeof($rows); $i++){
				$ret = $country->insert($rows[$i]->last_update,$rows[$i]->name);
	...
	function bash_update(){
		...
		if(isset($_REQUEST["json"])){
			...
			$result = "[";
			for($i = 0; $i < sizeof($rows); $i++){
				$ret = $country->update($rows[$i]->server_id,$rows[$i]->last_update,$rows[$i]->name);
				$ret->client_id = $rows[$i]->client_id;
				if($i > 0){
					$result .= ",";
				}
	...
	function get_for_client_update(){
		...
		if(isset($_REQUEST["json"])){
			...
			$country = new CountryModel();
			$result = $country->get_for_client_update($obj->last_update_time, $obj->page_number, $obj->page_size);
