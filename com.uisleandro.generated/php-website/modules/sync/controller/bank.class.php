...
use Sync\Model\Bank as BankModel;
...
class Bank {
	...
	public function get_for_client_insert(){
		...
		if(isset($_REQUEST["json"])){
			...
			$bank = new BankModel();
			$result = $bank->get_for_client_insert($obj->server_id, $obj->page_number, $obj->page_size);
	...
	function bash_insert(){
		$error_msg = "{\"status\":\"error\", \"usage\":\"[{\\\"client_id\\\" : (long),\\\"last_update\\\" : (datetime),\\\"code\\\" : (smalltext),\\\"name\\\" : (smalltext)}]\"}";
		if(isset($_REQUEST["json"])){
			...
			for($i = 0; $i < sizeof($rows); $i++){
				$ret = $bank->insert($rows[$i]->last_update,$rows[$i]->code,$rows[$i]->name);
	...
	function bash_update(){
		$error_msg = "{ \"status\" : \"error\" , \"usage\" : \"[{\\\"client_id\\\" : (long),\\\"server_id\\\" : (long),\\\"last_update\\\" : (datetime),\\\"code\\\" : (smalltext),\\\"name\\\" : (smalltext)}]\"}"; 
		if(isset($_REQUEST["json"])){
			$rows = json_decode($_REQUEST["json"]);
			if(NULL == $rows || empty($rows)){
				echo $error_msg;
			...
			$result = "[";
			for($i = 0; $i < sizeof($rows); $i++){
				$ret = $bank->update($rows[$i]->server_id,$rows[$i]->last_update,$rows[$i]->code,$rows[$i]->name);
				$ret->client_id = $rows[$i]->client_id;
				if($i > 0){
					$result .= ",";
				}
	...
	function get_for_client_update(){
		...
		if(isset($_REQUEST["json"])){
			...
			$bank = new BankModel();
			$result = $bank->get_for_client_update($obj->last_update_time, $obj->page_number, $obj->page_size);
